...
class RealRPPGAnalyzer:
    ...
    def __init__(self):
        ...
        # MediaPipe Face Mesh 지연 초기화 (Lazy Initialization)
        ...
        logger.info("✅ RPPG 분석기 초기화 완료 (MediaPipe 지연 초기화)")

    def _initialize_mediapipe(self):
        """MediaPipe Face Mesh 지연 초기화"""
        if self._face_mesh_initialized:
            return
            
        try:
            logger.info("🎭 MediaPipe Face Mesh 초기화 시작...")
            start_time = time.time()
            
            # MediaPipe를 여기서 import하여 서버 시작 시 오류 방지
            ...
            logger.info(f"✅ MediaPipe Face Mesh 초기화 성공 ({init_time:.1f}ms)")
            
        except Exception as e:
            logger.error(f"❌ MediaPipe Face Mesh 초기화 실패: {e}")
            self.face_mesh = None
            self._face_mesh_initialized = True  # 재시도 방지
